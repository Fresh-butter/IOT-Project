...
from app.utils import handle_exceptions, normalize_timestamp, format_timestamp_ist
...
async def create_log(
    ...
    if "timestamp" not in log_dict or log_dict["timestamp"] is None:
        log_dict["timestamp"] = get_current_utc_time()
...
async def update_log(
    ...
    updated = await LogOperations.update(id, update_data)
    if not updated:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, 
                          detail="Log not found")
        
    updated_log = await LogOperations.get_by_id(id)
...
async def get_logs(
    ...
    logs = await LogOperations.get_all(limit=limit, skip=skip, is_test=is_test)
...
async def get_log(
    ...
    """Get a log by ID"""
    log = await LogOperations.get_by_id(id)
    if not log:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, 
                          detail="Log not found")
    return LogInDB(**log)
...
async def get_logs_by_train(
    ...
    logs = await LogOperations.get_by_train_id(train_id, limit)
    return [LogInDB(**log) for log in logs]
...
async def get_logs_by_rfid(
    ...
    logs = await LogOperations.get_logs_by_rfid(rfid_tag, limit)
    return [LogInDB(**log) for log in logs]
...
async def get_logs_in_time_range(
    ...
    # Normalize input times to UTC for database query
    start_time_utc = normalize_timestamp(start_time)
    end_time_utc = normalize_timestamp(end_time)
    
    logs = await LogOperations.get_logs_in_time_range(train_id, start_time_utc, end_time_utc)
    return [LogInDB(**log) for log in logs]
...
async def get_latest_log(
    ...
    log = await LogOperations.get_latest_by_train(train_id)
    if not log:
        return None
    return LogInDB(**log)
...
async def get_logs_last_n_hours(
    ...
    logs = await LogOperations.get_last_n_hours_logs(train_id, hours)
    return [LogInDB(**log) for log in logs]
...
async def delete_log(
    ...
    """Delete a log by ID"""
    deleted = await LogOperations.delete(id)
    if not deleted:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, 
                          detail="Log not found")
